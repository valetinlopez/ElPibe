...
- **Display / Headings**: `"Bebas Neue"` o `"Anton"` (fallback: `system-ui, sans-serif` bold) — tipografía de número de camiseta, condensada, con carácter. Usar SOLO en: título de pantalla, nombre del jugador en el header de perfil, números grandes de stats (ej. "23" goles).
...
|---|---|---|---|---|---|
| `display-xl` | 40px | 44px | 700 | Bebas Neue | Splash, número hero de stat principal |
| `display-lg` | 32px | 36px | 700 | Bebas Neue | Nombre del jugador en header de perfil |
| `display-md` | 24px | 28px | 700 | Bebas Neue | Títulos de pantalla (ej. "MI PERFIL") |
...
- Usar unidades relativas (`flex`, `%`) en vez de anchos fijos en píxeles siempre que sea posible.
- Tipografía no escala con el tamaño de pantalla (se mantiene la escala fija de la sección 2), pero sí respeta el ajuste de accesibilidad del sistema operativo (Dynamic Type / Font Scale) hasta un máximo de 130%.
- Todos los tap targets interactivos (botones, íconos, chips clickeables) deben tener un área mínima de `44x44px`, incluso si el elemento visual es más pequeño (usar `hitSlop` en React Native).
- El radar chart y las cards de stats deben usar `Dimensions.get('window').width` para calcular su tamaño proporcional en lugar de un valor fijo, garantizando que se vean completos sin scroll horizontal en ningún dispositivo objetivo.
- Orientación: la app está diseñada solo para **portrait**. Bloquear landscape en `app.json` (`"orientation": "portrait"`).
...
## 12. Resumen de Tokens (para copiar a `theme.js` / `tailwind.config.js`)
...
export const typography = {
  displayFont: "BebasNeue-Regular", // o "Anton-Regular"
  bodyFont: "Inter-Regular",
  bodyFontMedium: "Inter-Medium",
  bodyFontSemiBold: "Inter-SemiBold",
};
```

---

## 13. Checklist de Implementación para Stitch / v0

Al generar pantallas con esta guía, verificar siempre:

- [ ] Fondo base es `#0B0E14`, nunca blanco ni gris claro.
- [ ] El azul metálico aparece en máximo 1-2 elementos por pantalla.
- [ ] Hay como máximo un botón primario (pill, azul) visible por pantalla.
- [ ] Tipografía de títulos/nombres usa estilo condensado tipo dorsal (uppercase, tracking leve).
- [ ] Todas las cards usan `radius-lg` (16px) salvo excepciones documentadas.
- [ ] El microcopy está en voseo argentino, tono de barrio, no corporativo.
- [ ] Los tap targets son de mínimo 44x44px.
- [ ] La pantalla está optimizada para portrait, ancho 360–428px, con `SafeAreaView`.
- [ ] No hay sombras negras planas: se usa glow de borde o `elevation-card` definido en sección 4.2.
- [ ] Los estados vacíos tienen ilustración outline + texto con personalidad + CTA si corresponde.
